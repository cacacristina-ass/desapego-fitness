...
var CHAVE_VALIDA = /^(img|thumb)\/[\w.\-]{1,120}\.(jpe?g|png|webp)$/i;
var fotos_default = {
  async fetch(req, env) {
    if (req.method === "OPTIONS") return new Response(null, { status: 204, headers: CORS });
    const chave = decodeURIComponent(new URL(req.url).pathname.slice(1));
    if (!CHAVE_VALIDA.test(chave)) return resposta(404, "N\xE3o encontrado");
    if (req.method === "GET" || req.method === "HEAD") {
      const obj = await env.FOTOS.get(chave);
      if (!obj) return resposta(404, "N\xE3o encontrado");
    ...
    if (!env.SEGREDO || auth !== `Bearer ${env.SEGREDO}`) return resposta(401, "N\xE3o autorizado");
    if (req.method === "PUT") {
      const tipo = req.headers.get("Content-Type") || "image/jpeg";
      if (!/^image\//.test(tipo)) return resposta(415, "S\xF3 aceito imagens");
    ...
    return resposta(405, "M\xE9todo n\xE3o permitido");
  }
};
...
// .wrangler/tmp/bundle-FPRqbK/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = fotos_default;
...
// .wrangler/tmp/bundle-FPRqbK/middleware-loader.entry.ts
